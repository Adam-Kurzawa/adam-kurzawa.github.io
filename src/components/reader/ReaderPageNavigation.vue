...
const props = defineProps([ 'metadata', 'chapterNumber' ])
...
const nextPage = () => router.push({ name: 'reader', params: { title: route.params.title, chapter: Math.min(props.metadata.chaptersCount, props.chapterNumber + 1) }, type: route.params.type })
const nextPageEnabled = computed(() => props.chapterNumber !== props.metadata.chaptersCount)

const hasMultiplePages = computed(() => props.metadata.chaptersCount > 1)
</script>
